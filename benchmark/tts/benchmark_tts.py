...
compute_ci = shared_metrics.compute_ci
...
def _benchmark_model_worker(model_config: dict, data_sample_dict: dict, config: dict) -> dict:
    ...
    if not tts_client.health_check():
        logger.error(f"Error: {model_name} service is not responding!")
        return {
            'model': model_name,
    ...
    if not asr_client.health_check():
        logger.error("Error: Whisper ASR service is not responding!")
        return {
            'model': model_name,
    ...
    if len(successful) > 0:
        # Calculate WER with CI
        wer_ci = compute_ci(successful['wer'].values)
        summary.update({
            'wer_mean': wer_ci['mean'],
            'wer_ci_lower': wer_ci['ci_lower'],
            'wer_ci_upper': wer_ci['ci_upper'],
            'wer_median': wer_ci['median'],
            'wer_std': wer_ci['std'],
            'wer_min': wer_ci['min'],
            'wer_max': wer_ci['max'],
        })
        
        # Calculate CER with CI
        cer_ci = compute_ci(successful['cer'].values)
        summary.update({
            'cer_mean': cer_ci['mean'],
            'cer_ci_lower': cer_ci['ci_lower'],
            'cer_ci_upper': cer_ci['ci_upper'],
            'cer_median': cer_ci['median'],
            'cer_std': cer_ci['std'],
            'cer_min': cer_ci['min'],
            'cer_max': cer_ci['max'],
        })
        
        # Calculate RTF with CI
        rtf_ci = compute_ci(successful['rtf'].values)
        summary.update({
            'rtf_mean': rtf_ci['mean'],
            'rtf_ci_lower': rtf_ci['ci_lower'],
            'rtf_ci_upper': rtf_ci['ci_upper'],
            'rtf_median': rtf_ci['median'],
            'rtf_min': rtf_ci['min'],
            'rtf_max': rtf_ci['max'],
        })
        ...
        # Calculate MOS with CI if available
        if 'mos' in successful.columns:
            mos_ci = compute_ci(successful['mos'].values)
            summary.update({
                'mos_mean': mos_ci['mean'],
                'mos_ci_lower': mos_ci['ci_lower'],
                'mos_ci_upper': mos_ci['ci_upper'],
                'mos_median': mos_ci['median'],
                'mos_std': mos_ci['std'],
                'mos_min': mos_ci['min'],
                'mos_max': mos_ci['max'],
            })
...
class TTSBenchmark:
        ...
        with ProcessPoolExecutor(max_workers=self.max_parallel_models) as executor:
            ...
            for future in as_completed(future_to_model):
                ...
                try:
                    ...
                    if 'error' in summary:
                        logger.error(f"Error: {model_name} failed: {summary['error']}")
                    else:
                        logger.info(f"{model_name} completed successfully")
                        
                except Exception as e:
                    logger.error(f"Error: {model_name} raised exception: {e}")
    ...
    def _generate_visualizations(self, logger):
        ...
        try:
            ...
            logger.error(f"Error: Visualizations failed: {e}")
